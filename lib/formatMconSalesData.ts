...
const approximatelyEqual = (v1: number, v2: number, epsilon = 1e-10) => Math.abs(v1 - v2) < epsilon;

// 2948.4307702418932
export const getMconDesignerRewards = (tokenAmount: number) => {
  const mconDesignerRewards: Record<string, number> = {};
  ...
  for (const p of MCON_PRODUCTS) {
    for (const d of p.designers) {
      ...
      designerNames[address] = d.name;
      mconDesignerRewards[address] =
        (mconDesignerRewards[address] || 0) + d.contributionShare * p.rewardShare * tokenAmount;
    }
  }

  const total = Object.values(mconDesignerRewards).reduce((acc, v) => (acc += v), 0);
  if (!approximatelyEqual(total, tokenAmount)) {
    throw new Error(`INCORRECT MCON DESIGNER CALCULATION: ${{ total, tokenAmount }}`);
  }

  return { mconDesignerRewards, designerNames };
};
